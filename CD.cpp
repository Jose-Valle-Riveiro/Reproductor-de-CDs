...
void CDs::LeerCarpeta(std::string direccion, std::list<CD_ELEMENTS>& Lista, bool &LecturaValida)
{
...
void CDs :: ListaCDs(std::list<CD_ELEMENTS>& Lista, std::queue<Cancion>& Cola) {
	...
	while (true) {
		...
		//MANEJO DE ERRORES
		if (std::cin.fail() || opcion < 1 || opcion > 13)
		{
			std::cin.clear();
			Console::Clear();
...
void CDs::CheckCola(std::queue<Cancion> Cola) {
...
void CDs::SortByArtista(std::queue<Cancion>& Cola, int Opcion) {
...
void CDs::SortByNombre(std::queue<Cancion>& Cola, int Opcion) {
...
void CDs::SortByDuracion(std::queue<Cancion>& Cola, int Opcion) {
...
void CDs::PonerCancionActual(Cancion* Rola) {

	 Reproduciendo = Rola;
}

Cancion* CDs :: ReturnCancionActual() {

	return Reproduciendo;

}

void CDs::ReproduccionActual(Cancion* Rola) {
...
void CDs::ReproduccionSiguiente(std::queue<Cancion>& Cola) {
...
void CDs::Imprimir(std::queue<Cancion>& Cola) {
...
void CDs::Reinicio(std::queue<Cancion>& Cola, std::list<CD_ELEMENTS>& Albums) {
